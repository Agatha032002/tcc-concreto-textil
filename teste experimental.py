...
def dados_de_entrada():
    while True:
      ...
      #  Ms = float(input('Digite o momento fletor que a viga deve resistir (Ms) em kN·cm: '))
        ...
        ca = float(input('Digite a quantidade de camadas: '))
        ...
        if confirma.startswith('s'):
            return bw, h, d, dc, Ec, As, Asc, Es,ca
...
def escolher_têxtil():
    ...
    while True:
        ...
        if op == 1:
            ...
            return 70, 0.0033, 100, 0.2,0.05,"AF - 0200 BR da FiberTEX" # Ef, Af1, σf, ea,et
...
def dimensionar_reforço(h,ea,et, Af1,bw, Ec,Ef,As,Asc,d,Es,dc,σf,ca):
...
def main():
    bw, h, d, dc, Ec, As, Asc, Es,ca = dados_de_entrada()
    Ef, Af1, σf, ea, et,nome = escolher_têxtil()
    x, Ix, Mr = dimensionar_reforço(h,ea,et, Af1,bw, Ec,Ef,As,Asc,d,Es,dc,σf,ca)

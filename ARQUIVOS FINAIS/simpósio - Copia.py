...
def dados_de_entrada():
    while True:
        ...
        if confirma.startswith('s'):
            return bw, h, d, dc, Ec, As, Asc, Es, Ms
...
def dimensionar_reforco(bw, h, d, dc, Ec, As, Asc, Es, Ms,Ef, Af1, ff,ea,et,nome):
    camadas = 1
    while True:
...
def main():
    bw, h, d, dc, Ec, As, Asc, Es, Ms = dados_de_entrada()
    Ef, Af1, ff, ea, et,nome = escolher_textil()
    x, Ix, Mr, camadas = dimensionar_reforco(bw, h, d, dc, Ec, As, Asc, Es, Ms,Ef, Af1, ff,ea,et,nome)
    if x is None:
